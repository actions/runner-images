...
if (Test-Path $ndkRoot) {
    ...
    Write-Host "LTS NDK $ndkLTSMajorVersion is not installed at path $ndkRoot"
    exit 1
}
...
if (Test-Path $ndkLatestPath) {
    ...
    Write-Host "Latest NDK $ndkLatestMajorVersion is not installed at path $ndkLatestPath"
    exit 1
}
