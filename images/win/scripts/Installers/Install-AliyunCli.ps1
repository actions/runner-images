...
$installerFile = "aliyun-cli-windows"
...
$packagePath = Start-DownloadWithRetry -Url $downloadUrl -Name "$installerFile.zip"
...
$externalHash = (Invoke-RestMethod -Uri $hashURL).ToString().Split("`n").Where({ $_ -ilike "*$installerFile*" }).Split(' ')[0]
